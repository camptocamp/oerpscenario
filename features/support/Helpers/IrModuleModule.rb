...
begin
  if Object.const_defined? 'IrModuleModule'
    ...
    IrModuleModule.class_eval do
      @openerp.log.debug("Extending  #{self.class} #{self.name}")
      ##########################################################################
      ...
      def self.update_needed_modules()
        @openerp.log.debug("DEBUG : update_needed_modules is deprecated")
      ...
      if Object.const_defined? 'ModuleQualityCheck'
        ...
        def self.run_base_quality_test(modules)
          @openerp.log.info("INFO : Function not available")
        end
      end
    ...
    @openerp.log.debug("IrModuleModule helper not initialized")
  end
rescue Exception => e
  @openerp.log.fatal("ERROR : #{e.to_s}")
end
