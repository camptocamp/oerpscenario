...
from support import model, puts
import datetime
...
def set_rates(ctx):
    for row in ctx.table:
        ...
        ctype_id = False
        if row['type']:
            ctype = model('res.currency.rate.type').get([
                ('name', '=', row['type'])
            ])
            assert ctype
            ctype_id = ctype.id
        curr_rate = model('res.currency.rate').browse([
            ('name', '=', date),
...
@given(u'I want to generate account chart from chart template named "{name}" '
       u'with "{digits}" digits for company "{company_name}"')
def generate_chart(ctx, name, digits, company_name):
